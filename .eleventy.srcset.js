...
module.exports = function(eleventyConfig, options) {
  ...
  eleventyConfig.addTransform('autoSrcset', async (content, outputPath) => {
    if (config.autoselector && outputPath && outputPath.endsWith(".html")) {
    ...
    if (config.jsonselector && outputPath && outputPath.endsWith(".json")) {
      let parsedContent = JSON.parse(content);
      let dom = new JSDOM();
      let frag = JSDOM.fragment(parsedContent[config.jsonselector]);
      ...
      if (images.length > 0) {
        ...
        for(let i = 0; i < frag.children.length; i++) {
          newContent += frag.children[i].outerHTML;
        }

        parsedContent[config.jsonselector] = newContent;
  ...
  const generateSrc = function(image, width = config.fallbackWidth, height = config.fallbackHeight, cropPosition = config.cropPosition) {
    return config.resizeOriginal ? resizeSingleImage(image, width, height, cropPosition, false) : resizeSingleImage(image);
  }
  ...
  const updateExistingImg = async imgElem => {
    ...
    if (config.createCaptionFromTitle && imgElem.getAttribute('title')) {
      imgElem.insertAdjacentHTML(
        'afterend',
        `<figure>
          <img alt="${ imgElem.alt || '' }" src="${ imgElem.src }" srcset="${ imgElem.srcset || null }" sizes="${ imgElem.sizes || null }" />
